...
**⚠️ THE BIG ONE: FIVE THINGS ARE UNWALKED ON THE HANDSET, and they are the first thing to do.**
Nothing below is verified on an iPhone; several are not verified anywhere. In order of how likely they are
to be wrong:

1. **The phase bookmark (`de4309e8`) — UNWALKED ANYWHERE.** Press the arrow → it gets a frame and the phase
   it sits on lights up → hold and slide → the highlight follows → lift and it stays. The bundle compiles,
   the app mounts, 129/129 shared tests pass, and *nobody has driven it end to end*, because the Expo web
   target signed itself out partway through the run and signing in is the founder's act. Four designs were
   tried before this one (§0, and the superseded row beneath it); if it is wrong again, read that row
   before proposing a fifth — the three rejections rule out more than they look like they do.
2. **The iOS `onPanResponderTerminationRequest` fix (row in §0) — reasoned, not reproduced.** No surface
   available in the agent's sandbox *has* the bug. It is the third of its exact family this session.
3. **The prepare STEER (`RollWheel`'s external-value fixes)** — the wheel itself is walked on Expo web
   (▼ lands on the row, a drag commits, class and list follow). The steer it exists to serve is not: it
   needs a **real generation**, which spends a trial chapter on the founder's live account.
4. **The section-card "+" return leg** — same reason: needs a real generation to come back from.
5. **The clay phase rules (`10d0516b`)** — seen on Expo web, cheap to confirm on the handset while doing
   the rest.
...
**Where §2 picks back up**, once the handset walk is done: the two **pick screens**, then **add a subject**
(Q3 answered: add mode only), then the **check-mood window**, which needs **Q9** — the last unanswered
founder question in §4, already put to him ahead of its stage.
