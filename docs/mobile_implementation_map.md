...
open. Of the three things that closed it, **the check-mood window landed on 2026-09-16** and **"add a
subject" is HELD to 6a by the founder's call that day** (there is no door to it on the phone until the
Settings accordion exists — the same reasoning as Q1's HOLD). What is left: the **"Which class?" screen** (built, never DRAWN — his profile has one
class per subject, so it is correctly skipped every time; add a second class to either subject and the
path opens), **add a subject** (Q3 answered: add mode only), and the **check-mood window**, which needs
**Q9** — the last unanswered founder question in §4, already put to him ahead of its stage. After 5d the
map is linear again: **5e** first run, then **6a** the shell layer, which is what lights the Settings gear
and Ask Meyy.
...
**Due next:** none blocking. Q9 dissolved 2026-09-15 when the tour was un-deferred and Q22 was answered on 2026-09-16 (add-a-subject HELD to 6a); the next questions bind at 5e (Q5, Q21) and 6a (Q6, Q7, Q8).
