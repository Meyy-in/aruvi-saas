...
|---|---|---|---|---|---|---|---|---|
| `mobile_implementation_map/01-shell.md` | page.jsx shell · layout · GuidedTour · ProfilePortal | 95 | 85 | 0 | 0 | 0 | 5 | 5 |
| `mobile_implementation_map/02-profile.md` | TeachingProfile · wheels · shared budget | 98 | 75 | 0 | 0 | 2 | 17 | 4 |
| `mobile_implementation_map/03-firstrun-login.md` | Login · PrivacyNotice · Agreement · FirstRun | 122 | 119 | 0 | 0 | 0 | 0 | 3 |
| `mobile_implementation_map/04-settings-askmeyy.md` | Settings (7 subviews) · Ask Meyy · Dropdown | 124 | 117 | 0 | 0 | 0 | 0 | 7 |
| `mobile_implementation_map/05-lists-prepare.md` | My Classes · My Lessons · Prepare · Year Plan | 128 | 110 | 0 | 0 | 0 | 11 | 7 |
| `mobile_implementation_map/06-lessonview.md` | LessonView · ChapterOrg · Assess · Bookmark · Notes | 129 | 117 | 5 | 0 | 1 | 3 | 3 |
...
| **7·preview** | **A DOCUMENT IS SHOWN BEFORE IT IS SENT** (founder, 2026-09-17, holding the Files app beside ours: *"it displays the pdf on the full screen and from there gives option to choose. Whereas our current export does not display and directly opens app"*). The share sheet was never wrong — his own screenshot had Mail, WhatsApp, AirDrop and Save to Files all one tap away — but it arrived BEFORE he had seen what he was about to send, so the first thing Meyy did with a document it had just built was ask where to put it. ★ **The document now comes first and sharing is a choice made from it**: `app/(app)/preview.jsx` renders the PDF full-screen with the Year Plan's own export arrow in the frozen bar, and the sheet is raised only when she taps it. She can also just close it, which she previously could not do without first dismissing a sheet she had not asked for. ★ **IT IS OUR SCREEN, NOT THE SYSTEM'S, AND THAT WAS FORCED.** The screen he photographed is iOS's Quick Look (`QLPreviewController`), which would preview Word too — but there is **no `expo-quick-look` on npm**, and `react-native-file-viewer`, the usual answer, was last published in **2021**, before the new architecture this app runs (`newArchEnabled: true`, RN 0.86). Quick Look therefore means hand-written native code AND leaving Expo Go, which this app is still on (`app.json` carries no `expo-dev-client`). Against that, an in-app preview keeps her inside Meyy under Meyy's own bar with the bottom nav still up — the founder's own rule that a screen must not take the app's nav away and leave one way out of itself. ★ **When this app moves to a dev build, Quick Look is the upgrade to revisit** — it is the only thing that would give a `.docx` the same treatment. ⚠️ **PDF AND iOS ONLY, and the test lives in ONE place** (`canPreview`, lib/download.js): not the web target, where the browser's download IS the gesture; not Android, whose WebView has no PDF renderer and would show a blank page or offer to download it again; not Word, which no WebView can draw. **So the YEAR PLAN, being Word-only, still goes straight to the sheet** — that raggedness is named rather than hidden, and closing it means giving `POST /api/year-plan/export-docx` a PDF format. ★ **THE PRIMITIVE SPLIT, AND THE FILE'S LIFE MOVED WITH IT.** `downloadDocument` was fetch → share → delete in one `finally`, which is right when the sheet is the end of the story and wrong when a screen must hold the file. It is now `requestDocument` (the call, the error unwrap, the name) → `fetchDocument` (writes the courier copy) → `shareFile` → `discardFile`, with `downloadDocument` composed from them so **Settings' three documents are byte-for-byte unchanged**. ⚠️ On the preview path the cache file is deleted on UNMOUNT — by the ✕, the back gesture, the bottom nav, any way out there is. ★ **The bar names the ITEM, not the file** (the Settings bar's rule of 2026-09-03): it reads "Force and Pressure", the words she chose the report by, while the FILENAME stays what it was and is what travels into Mail — a 40-character slug in 19px display serif clips to nothing and tells her less. ⚠️ `allowingReadAccessToURL` is not optional: WKWebView will not open a `file://` it has not been granted, and a PDF that silently renders blank is exactly the failure that reads as "the export is broken". ⚠️ `/preview` keeps **My Lessons** lit — a report OF a lesson, reached from its card, has not left the repository | *this commit* | 🟡 **Static clean; HANDSET WALK OWED, and the FIRST thing it proves is whether Expo Go carries `react-native-webview` at all.** It is in Expo's own `bundledNativeModules.json` for SDK 57 at **13.16.1**, which is the signal that it is bundled — but it could not be confirmed against the docs from here (the proxy forbids `expo.dev`; the npm registry is reachable, which is how it was installed: `npx expo install` fails on the Expo API, so it went in as `npm install --save-exact react-native-webview@13.16.1`, the version that file pins). ⚠️ **Metro must be restarted** — a new native module is not a Fast Refresh. babel-parse clean on all seven touched files; the parity checker is unmoved at its pre-existing 26/40. **The walk:** Reports → PDF → Download → the report FILLS the screen under a bar reading the chapter's name → the arrow raises the sheet → ✕ returns to My Lessons with it still lit. Then Reports → **Word** → Download must still go straight to the sheet, unchanged |
| **map·reconcile** | **THE INVENTORY HAD STOPPED BEING TRUE, AND THE RULE MADE THAT WORSE** (2026-09-17). Appendices **03** and **04** were drawn 2026-09-15 and never amended when 5e, the front-door pass, 6b (A-H), 6c, F9 and F11 all landed on 2026-09-16 — so ~120 rows still read MISSING for shipped work. ★ **The danger was not the staleness; it was this file's own rule.** "The rows are the truth, never the table" is right when the rows are maintained, and it INVERTS the moment they are not: a session following it would have re-ported finished screens on the doc's own authority. Both appendices were walked row by row against the mobile code and corrected — **71 rows in 03, 50 in 04** — each carrying a dated `★ 2026-09-17 reconcile:` clause naming the file that closed it, so the evidence travels with the row. ★ **The SubscribeFlow reversal was the sharpest case:** ten rows still said NOT-PORTED-BY-DECISION, a status that asserts a *decision* the founder had already reversed with Q11 — those read worse than MISSING, because they tell the next reader not to build the thing he asked for. Also struck: **Q14** and **Q16**, both answered by the build and never crossed off; and **step 6c had no header marker at all** while its own 25 rows said DONE, which reads as not-started. ⚠️ **The lesson is about MAINTENANCE, not counting** — moving a step header to DONE is not the same act as amending the rows under it, and 6b did the first without the second. The header table is now tallied BY SCRIPT from the live rows, with an `Other` column that exposes two bits of vocabulary drift (app. 05's eight `BUILT`, app. 02's one `REMOVED FROM THE WEB`) | *this commit* | ✅ **Verified structurally**: 121 rows rewritten with a pipe-aware splitter, zero rows added or removed, and the Feature / Web-ref / Mobile-ref / Depends-on columns confirmed byte-identical before and after — only the status cell and the notes moved |
| **map·gaps** | **AND THE FOUR GAPS IT FOUND WERE CLOSED, NOT JUST RECORDED** (founder, 2026-09-17: fix all three). The reconcile's value was the short list it left behind, and it was short enough to act on the same day. ⓵ **The privacy version line** (app. 03 row 69, app. 04 G4) — the phone printed `{title} · version {v} · effective {date}` where the web ends `Version {v} · {published} · English · This notice is available at any time under Settings › Legal.`; ★ **two surfaces quoting a legal document in different words is the one place a teacher is entitled to identical text**, so matching beat improving and the phone's invented `· current version {n}` tail went with it — the accepted block above already says a newer version applies, and saying it twice differently on a legal screen is worse than saying it once. ⓶ **The agreement hint** said "confirm the five points" against the web's "confirm each point" — which also hard-coded a COUNT the document's own structure supplies, so a sixth acknowledgement would have made it false. ⓷ **`?version=` and the older-version hint** are ported to both phone privacy surfaces (`components/PrivacyNotice.jsx` takes the prop; `app/privacy.jsx` reads it off the route). ⚠️ **AND THAT ONE IS REACHABLE ON NEITHER SURFACE** — all three web call sites take the default, so it has never fired in production. It was ported so the two components' APIs cannot drift (§3), not because a caller waits: **wire a caller or delete it on BOTH, never leave it half-alive on one.** ★ **Two founder decisions recorded the same day, each at the rule rather than in prose:** a subject she has BOUGHT is **permanent for its term** (app. 02 question 6 — the cost was named and accepted: a wrong purchase stays all year, and the remedy, if support traffic shows one, is a "stop teaching this, keep the subscription" door in the removal redesign, NOT a softening of `subjectSurvivesEmpty`); and at 8b **the TOUR's check-window trigger is the one that goes**, not first run's one-shot — ★ not seniority but REACH: the tour's ending can only fire for a teacher who RAN the tour, and Meyy assumed just as much for the one who skipped it. ⚠️ **The web still has only the tour's trigger, so on the web that disclosure is still missed by everyone who declines** — web owed, recorded | *this commit* | ✅ **babel-parse clean on all six touched files**; the four changed strings diffed against `web/app/components/{PrivacyNotice,Agreement}.jsx` and confirmed identical. 🟡 **Unwalked** — the walk is Settings › Legal on both pills, and the OTP screen's Privacy Notice link |
| **map·05** | **AND APPENDIX 05 WAS STALE THE SAME WAY** (2026-09-17). Reconciled before building, precisely so nothing was rebuilt: **25 rows** were wrong, and the pattern was the one 03 and 04 had shown. ⓵ The **return-with-section** cluster (A9, B17, D12) still read DEFERRED although it shipped on 2026-09-16 and the founder WALKED IT ON HIS IPHONE. ⓶ The **cutover** rows (B8, B9) read MISSING although `YearNudge.jsx` carries them. ⓷ The report sheet and Year Plan export (C30-C36, E8, E9) sat on a `BUILT` status that is not in the vocabulary at all. ⓸ **D4 was not owed but INAPPLICABLE** — `startPreparing` returns nothing and the wait is always drawn on the card, so the web's in-place fallback has no case here; re-classed NOT-PORTED-BY-DECISION rather than left looking like debt. ★ **So step 7's "four clusters" were really three**, and saying so before building was worth more than the building | *this commit* | ✅ 107/127 DONE; the two left (B6, B7) are copy and a style key, both outside step 7 |
| **7·clusters** | **THE REST OF STEP 7 — prior years, the teaching ledger, and six quiet guards** (app. 05 A4-A7, A17, B13-B16, B19-B24, C9, C37, D9). ★ **THE GUARDS WERE ONE BUG WEARING SIX FACES, and the root was a single line**: `indexPlans` did `(rows || [])`, flattening "this device has never asked" (`cachedPlans` → null) into `{}` — indistinguishable from "asked, and she has none". So a bound section on a cold cache drew the sand "Pick a chapter to begin" card, and the picker told a teacher with a full shelf she had nothing prepared. **`undefined` is a state, and it was being thrown away at the indexer.** That is the Support `metaErr` rule of 2026-08-27 — *a screen may say it does not know; it may never invent an answer about her record* — reached through a third door. ⓶ **`pullSectionHistory` existed in shared with ZERO callers on this surface**, so `hasHistory` read a cache nothing ever filled and the glyph would have been wrong on a second device even once drawn. ⓷ **A4's re-pull makes A5's hold load-bearing**: a reconcile writes the server's answer over the local cache, so a tick fired mid-attach reverts the card under her hand — skipped, never queued. ⓸ **D9's message was being written to a screen that had already gone**: `router.navigate` fires before the read-back resolves, so the one case the verify exists for said nothing at all. ★ **PRIOR YEARS, and the line that governs it**: the folders are read-only in My Lessons (no archive, no report, no attach — sections belong to the year she is in NOW) and the ONE door back into current work is the "+" picker's own folder, where `attachPriorChapter` deliberately MAKES a chapter this year's. ⚠️ Both folder fetches filter to `prepared && !archived`, because **the library is SHARED** — an unfiltered `/plans?year_id=` read would offer her every sample plan Meyy owns as though it were her own work | *this commit* | ✅ **babel-parse clean on all five touched files; 203 shared tests green; the parity checker's coverage rose 328→338 with NO new disagreements**, which is the real check on the 17 new theme keys — ten of them map to a web class of the same name and all ten agree with it. ⚠️ **The keys were DERIVED from globals.css, not measured on a running web** (no dev server reachable from this session) — that checker result is why shipping them behind a named caveat was defensible, and a 390px re-measure is still owed. 🟡 **HANDSET WALK OWED on every one of these**: open a class with history and check the glyph and its popup; open the "+" picker on a cold start and watch for "Loading lessons…" rather than a false "none"; expand last year's folder in both places; bring a chapter forward and confirm it leaves the folder and gains its "version" stamp |
| **8·parity** | **THE PARITY DEBT IS PAID, AND THE CHECKER WAS PART OF THE DEBT** (app. 06; step 8 item 1). 37 value disagreements → **0**, and `node mobile/theme/check-parity.mjs` now exits 0. ★ **THE CHECKER WAS WRONG ABOUT TWO OF THEM AND THE PHONE WAS RIGHT.** `const PHONE = /max-width:\s*(600|640)px/` meant a rule inside `@media (max-width: 400px)` was DROPPED and the base rule reported as the winner — wrong at every width the phone runs at. `tp_stat_n` (21 vs 19) and `tp_stat_l` (8.5 vs 8) had sat in this map as debt since 2026-09-15 on that basis. The filter now takes any `max-width` the 390px reference satisfies, which immediately surfaced a REAL one the old regex had been hiding (`lgl_ack_title`, a ≤480 rule). ★ **AND A NAMED-DIVERGENCE TABLE REPLACES THE FOLKLORE.** The map carried "skip the 3 false positives" as prose; there are twelve, in three families, and each now sits in `NAMED_DIVERGENCES` with its reason, PRINTED on every run — a silent skip list rots, because nobody notices when a reason stops being true. The families: `.assess-flat` re-palettes items at a higher specificity than `targetsAlone()` will read (the phone renders flat, so ITS values are right); one web class becomes two RN keys because a `View` cannot carry a font; and the 16px on `.dd-btn`/`.dd-opt` is iOS Safari's INPUT-ZOOM GUARD, a browser workaround with no native counterpart. ⚠️ **The exit status now reflects section B only** — section A's ties are a report about the WEB's own cascade with nothing in `web.js` to change, so failing on them meant "run it until it is clean" could never be satisfied. Of the 20 real corrections, the one worth naming is `uv_durline`: it was the wrong FACE as well as the wrong size — body-italic 13 where the web is mono 11 | *this commit* | ✅ **`✓ 0 value disagreement(s) · 12 named divergence(s) · 25 web-CSS tie(s)`, exit 0** |
| **8·lessonview** | **FOUR BEHAVIOUR BUGS, AND THREE FOUNDER ANSWERS** (app. 06 rows 5, 16, 21, 26, 30, 46, 75, 81, 83, 86, 87, 93, 94, 95, 101, 124, 125). ⓵ **`writePointer` left `done` TRUE under a moved pointer** — undo from the final unit undid the completion on screen and left the chapter still filed as finished, because `done` is what paints the section card clay and what the history ledger reads. ⓶ **The bookmark reset was shown but never WRITTEN**, so after "Mark complete" the stored row still pointed into the old unit — and `pushSectionState` carries the stored row, so her other device inherited it. ⓷ **No resync on return**: `AppState` "active" is the phone's answer to the web's focus/pageshow/storage, and it adopts the stored row only when it belongs to the unit she is on. ⓸ **The bookmark centre was a literal `13`** — `uv_phase`'s own padding hard-copied into arithmetic, so re-measuring that style would have walked every centre off its row, and the checker cannot see a number that is not in `web.js`. ★ **Q19 moved the WEB** (24 uses of `#0f6e56` → `var(--pine)`), **Q18 renamed Speak → Dictate on both**, and **Q20 named two divergences rather than resolving them**. Ported alongside: the maths-prep flat list became a WINDOW with an SVG fade (RN has no `linear-gradient`), the accordion head learnt its subject, the legacy assess card got its chrome back, the listening passage became a marked block, and the forward nav now shares the panel's last row | *this commit* | ✅ babel-parse clean on all touched files; parity exits 0. 🟡 **HANDSET WALK OWED on every one of these** — mark a unit complete then undo it and confirm the chapter is no longer filed as done; move a bookmark, background the app, return; open a maths-prep chapter and confirm the list is a window; open an SS chapter and confirm the head is small and the count gone |
| **8·notes** | **THE JOTTING SAT ON THE RULE** (founder, 2026-09-17: *"the jotting in the chapter notes sits on the horizontal line — it should be a tad higher"*). ★ **THE NUMBERS ALREADY MATCHED THE WEB, AND THAT WAS THE PROBLEM.** Same 16/32, same `paddingTop: 5`, rules on the same 32px grid — but CSS splits the 16px of leading EVENLY above and below the glyphs (half-leading) while React Native seats them lower in the line box, and at a 32px pitch those few pixels are the whole gap between the writing and the rule beneath it. ★ **A PORTED MEASUREMENT IS NOT A PORTED RESULT when the two engines lay text out differently** — the same class of divergence as the 2026-09-13 font-scaling decision, and the only honest fix is to compensate rather than to match harder. ⚠️ The compensation is on the PADDING (`CN_TEXT_LIFT`, one constant to turn), so the text rises within a band whose rule stays at the band's foot; moving the RULES down instead would have restored this gap by stealing it from the line below | *this commit* | 🟡 **Unwalked — and the exact lift is a judgement**: 4px at a 32px pitch. If it still reads low, or now reads high, `CN_TEXT_LIFT` in `theme/web.js` is the only number to change |
| **otp·timer** | **A MISTYPED CODE WAS BLAMED ON THE CLOCK** (founder, 2026-09-17: *"when wrong code is put for otp, it says 'code has expired'… There is no timer too"*). ★ **THE BUG WAS ONE REGEX READING A WORD OUT OF A SENTENCE THAT DOES NOT MEAN IT.** Supabase answers a mistyped code and a dead one with the SAME string — *"Token has expired or is invalid"* — and `shared/auth.js` tested `/expired/i` against it, so a teacher who fat-fingered one digit was told her code had expired and sent to Resend. The word carried no information; the test only looked like it did. ★ **THE CLOCK IS THE ONLY HONEST DISCRIMINATOR**, so `verifyOtp` stops guessing (it returns `OTP_WRONG`, plus a rate-limit branch that does speak for itself) and each front door decides by its own countdown: a rejection while the timer runs is a WRONG code, after it a DEAD one. ★ **AND THE TIMER IS NOW ON SCREEN** — `OTP_TTL_MS`, **60s**, anchored to the moment the SMS was REQUESTED because that is when the server's expiry starts; anchoring it to when the screen appeared would drift by however long the send took, and that drift always runs in the direction that flatters us. **While it runs she sees the time left and is NOT offered Resend** (a fresh code would invalidate the one she is halfway through typing); **at zero the countdown, the Verify button and the round trip all go and Resend is the only thing left** — the founder's rule, one state one action. ⚠️ **`OTP_TTL_MS` IS A CLAIM ABOUT SUPABASE, NOT A SETTING** — and the first value was a GUESS that the project disagreed with. It was written as 60s on the founder's answer; the project was then actually asked (`GET /v1/projects/<ref>/config/auth`) and replied **`"sms_otp_exp": 300`**. ★ **THE SERVER HAD BEEN RIGHT ALL ALONG, AND THE GUESS WAS WRONG IN THE DIRECTION THAT LOOKS LIKE A BUG**: at 60 the screen would have killed a code the server was still accepting, told her to send a new one, and made a working sign-in look broken four minutes early. ★ **The general lesson, and this file keeps re-learning it: a number that describes SOMETHING ELSE'S behaviour is not a decision to take, it is a fact to look up.** The dashboard does not expose SMS OTP expiry at all (only the email one) — it is Management-API only, which is why nobody had ever seen this value. ★ **THE FOUNDER THEN CHOSE 60 DELIBERATELY** (2026-09-17, having seen the real figure), so the app is 60 and **the SERVER is changed to match** rather than the reverse. ⚠️ **THE APP IS A LIE UNTIL `sms_otp_exp` IS 60 ON THE PROJECT** — the `GET` that proves it is in `auth.js`'s own header. ★ **AND THE 60→300→60 ROUND TRIP LEFT A TRAP WORTH NAMING**: the front doors originally hid Resend until EXPIRY, which is kind at 60s and cruel at 300s (five minutes with nothing to press if the SMS never lands). The rule is now gated on `OTP_RESEND_LOCK_MS` instead — **30s**, deliberately BELOW the code's life, because equal values would make the unlock and the expiry land on the same instant and the mid-window Resend would never appear at all. ⚠️ `sms_max_frequency` must be **30** on the project to match, or the tap comes back refused. **A rule that is kind at one duration can be cruel at another, and changing the duration is exactly when nobody re-reads the rule.** | *this commit* | ⚠️ **AND THE TIMER WAS DEAD ON THE OTHER DOOR — found on the handset, 2026-09-17: *"the time left shown on mobile is stuck at 0:00 not counting down from 60"*.** The returning-device flow called `sendOtp` DIRECTLY instead of `requestOtp`, so `otpAt` was never set. Everything downstream reads `otpAt > 0`, so the screen showed **0:00 for ever** — and `otpDead` stayed **FALSE**, which is the cruel half: she was neither told how long she had NOR ever offered Resend, while the screen went on asking her to type a code it was reporting as gone. **The create door was walked; the returning door is the one every existing teacher uses.** ★ The fix is structural, not a second `setOtpAt`: `requestOtp` is now the ONLY caller of `sendOtp` on either surface (grep-verified, one call site each) and it RETURNS the error rather than writing it, because the two doors render errors on different lines and the old version wrote to `mobErr` from a screen that does not show it — so a failed send on the returning door was silent. Both resend links now surface their error too. **Identical bug on the web, fixed in the same edit** (§0 rule). | ✅ Both files parse and resolve clean (`check-scope.mjs`), one `sendOtp` call site per surface, 203 shared tests green. 🟡 **Still needs the rest of the walk** — type a wrong digit and confirm it says the code did not match; let it run out and confirm only "Send a new code" remains |
| **acct·keys** | **THE ERASE GATE OPENED BEHIND THE KEYBOARD** (founder, 2026-09-17, on the handset: *"when i press delete my account, the Erase window is hidden behind the keyboard"*). Both delete gates sit at the bottom of the LONGEST screen in the app and the first one AUTOFOCUSES its field, so the keyboard rose over the very warning the gate exists to make her read — on the one screen in Meyy where not reading the warning is irreversible. ★ **THE SECOND TIME THIS EXACT TRAP HAS BEEN FOUND BY HAND** — `support.jsx` on 2026-09-16, same shape, same fix, and its note already explains why this is `automaticallyAdjustKeyboardInsets` and NOT a `KeyboardAvoidingView` (a KAV lifts a card with nowhere to scroll; these are scrollers inside a Stack under two bars, where a KAV needs a `keyboardVerticalOffset` and starts guessing). The inset makes the gate REACHABLE; a `scrollToEnd` on open brings it into view so she is not left dragging for it, delayed 120ms because scrolling to an end that has not laid out yet lands short. ⚠️ **PHONE-ONLY, and named in the header per §0** — the web has no soft keyboard, so there is nothing to mirror. ⚠️ A sweep found the only other two ScrollViews with inputs and no keyboard handling (`prepare.jsx`, `ProfileEditor.jsx`); both are number-pad fields sitting HIGH in their content, which is a far weaker case, and both were left alone rather than changed unwalked — recorded here so the next person finds them without re-running the sweep | *this commit* | ✅ Parses and resolves clean. 🟡 **Unwalked** — open Delete my account and confirm the gate rides above the keyboard, then the same for the second window |
| **erase·newuser** | **AN ERASED NUMBER SIGNED UP AGAIN AND WAS TREATED AS A VETERAN** (founder, 2026-09-17, on the handset: *"i deleted 9000000002 account. when i entered as new user it did not show trial but took me directly to my classes page where a tour was waiting"*). ★ **THE SERVER SIDE IS CORRECT AND WAS NEVER THE PROBLEM** — `/data-rights/erase` destroys the account record, readiness, sections, plans, allocations, archives, years AND the entitlement (so `trial_chapters` resets, added to the walk on 2026-08-24 for exactly this reason); `/onboarding/known` answers false immediately; the tour came back because `tour_offered_at` lives ON the account record and went with it. **The tour was the system working.** ★ **TWO CLIENT MEMORIES SURVIVED THE ERASURE, both keyed to the phone rather than to the account.** ⓵ `firstRun.js`'s activation latch is in-memory and keyed by MOBILE NUMBER, which IS the identity — so a number that erases and signs up again inside the same app run matches a latch set by the account that no longer exists. `hasActivated()` then answers *"I watched her finish first run"* about a teacher the server has never met, the `_layout.jsx` gate stands down, and she lands in an EMPTY app with a tour running over nothing. Cleared in `clearSession`, the one choke point every door passes through. ⓶ `aruvi_device_seen` was never in `TEACHER_CACHE_PREFIXES`, so the handset still opened on **Sign in** — which is why no trial was shown: the *"Free to try · any 3 chapters"* card lives on the CHOOSE screen. Founder's call (2026-09-17): **erasure forgets the device, a log out does not** — a returning teacher should not be re-pitched every time she signs out. Now `forgetDevice()` in shared/signout, called from the erase handler on BOTH surfaces, with the key itself moved to shared so the two cannot drift. ⚠️ **AND A DESIGN FACT WORTH KNOWING RATHER THAN FIXING BLIND:** nothing reserves an erased id and nothing at signup consults `erasure_log` (its only reader has no callers), so **delete→rejoin resets the 3-chapter trial, unlimited times, on the same Supabase Auth user** — which is never deleted, there being no admin-API call anywhere in the repo. That is deliberate (*"a tombstone would itself be a remnant"*) and it is also the abuse vector; it needs a founder decision, not a patch. ⚠️ `web/app/components/AccountPanel.jsx` holds a SECOND erase handler that is rendered nowhere and would 400 (it omits `downloaded_confirmed`) — dead, left alone, recorded so it is not mistaken for live | *this commit* | ✅ 74 files parse and resolve clean, 203 shared tests green. 🟡 **Unwalked** — delete an account and confirm the phone opens on Choose with the trial card, and that signing up on that number lands in first run, not My Classes |
| **map·01-02** | **THE LAST TWO APPENDICES RECONCILED — and the unknown was smaller than feared** (2026-09-18). Appendix **01** had 43 stale rows and **02** had 23; every one now carries a dated `★ 2026-09-18 reconcile:` clause naming the file. ★ **Neither family was hiding a feature.** 01 is 85/95 DONE with nothing MISSING — the gear, the bar, Ask Meyy, first-run's gate, the tour, Settings and the account name had all shipped under it. 02 is 74/98 DONE with nothing MISSING. ⓵ **One real bug, fixed while reading**: `ProfileEditor`'s save-failed banner lived inside the BUDGET branch only, so a verified mismatch on a class, section, periods-a-week or duration save was silent — the exact outcome the read-after-write doctrine exists to announce. It is one `saveFail` element under every step's heading now. ⓶ Small: the empty class list lost its "at this stage" clause (restored); `lib/portal.js`'s `leavePortal` had no caller and a half-reset (deleted — lesson 4); row 33's ⚠️ described a web rule that does not exist (struck). ⓷ **What is left is a DECISION, not code** — app. 02 open question **8**: adding a subject on the phone sets its classes up and discloses afterwards; the web asks about every class. Seven rows (02: 31, 40–45) wait on it as DIVERGED. ⓸ **Web owed, recorded in the rows:** the set-up check fires only from `finishTour` (01·74), `/readiness` fetched directly rather than through the store (01·11), budgets written inline rather than through `setGradeBudget` (02·92). Phone owed: the brand mark 22px vs the web's 19 (01·26); `applyClasses` onto `finalizeSubject` (02·16). `npm run check:scope` clean; anchors unchanged (the two known `<Round>` wrappers). |
| **map·leftovers** | **THE HAND-OFF'S SMALL LIST, WORKED THROUGH** (2026-09-18, after the 01/02 reconcile). ⓵ **The set-up check reaches every new teacher on BOTH surfaces** — the one-shot moved to `@aruvi/shared/setupCheck` (`queueFirstRunCheck` / `takeFirstRunCheck`, same key, +1 test → shared 210 green); the web queues it in `onFirstRunComplete`, spends it on My Classes, and `finishTour` stopped raising the window. Both surfaces now RE-QUEUE when the one-second timer is cancelled — `take` spends the flag first, so a tap away in that second had lost the question for good. ⓶ **A paid subject with no classes** no longer breaks two web paths: the Prepare picker lists only subjects with a class (none → a sentence saying where to add one), and the data-rights export lists it as "— · —" in both Word and PDF instead of claiming "No teaching profile on record" (`tests/test_export_classless_subject.py`, 3). ⓷ `aruvi-scripts/entitlement.py grant` REFUSES scopes that name nothing Meyy offers (typo'd subject, bad stage, a stage the subject lacks) and grants nothing. ⓸ `web/app/components/AccountPanel.jsx` deleted — nothing imported it. ⓹ Phone: the bar's mark is 19px as the web's is at ≤600 (`BAR_CONTENT_H` 64 → 61); My Classes' empty state and its third welcome sentence match the web (app. 05 B6/B7); four more stale rows closed (03·6, 03·117, 04·I24, 05·E7). **Still open:** `?version=` (dead on both surfaces equally now — a product question, not a half-alive seam) and the 390px re-measure. All STATIC + unit-verified; the walk covers them. |
| **setup·model** | **THE SET-UP MODEL, SETTLED** (founder, 2026-09-18, answering app. 02 Q8 — full text at the foot of app. 02). Four changes made it true: ⓵ the first-run check waits for the tour to be done or no longer offered (web `page.jsx` effect + phone `index.jsx` focus effect, moved below `tourOnOffer` to avoid a TDZ read); ⓶ `entitlement.py grant` seeds the profile through checkout's own `_apply_subscription_profile` + `_purge_trial_artifacts` (walked on a scratch state dir: Science · Class 6 · 6A); ⓷ the "change?" queue asks only for a subscription's defaults — new `setupCheckAdds` in shared (+1 test, 211 green), used by both diffs, so Add › Class into an existing or emptied subject asks nothing; ⓸ "+ add a subject" opens subscribe on both surfaces, retiring the web's per-class add run (7 rows of app. 02 close as not-ported). |
| **7·preview·settings** | **THE INVOICE IS SHOWN BEFORE IT IS SENT** (founder, 2026-09-18: *"the invoice when pressed (PDF) should appear in the screen as it does for pdf LP export"*). Subscription & billing's invoice rows and Your data's "Download as PDF" now take the report window's path — `canPreview` → `fetchDocument` → `/preview`, the share sheet a choice from its arrow; Word still goes straight to the sheet. Both pass `from: "settings"`, which `(app)/_layout.jsx` reads so the bar lights nothing (a bare `/preview` means My Lessons). ⚠️ A preview no longer marks her data as "downloaded" for the delete flow's wording — seeing it is not holding a copy. Unwalked. |
| **billing·settling** | **A NEW INVOICE SHOWS AS A WAIT, NOT AS AN EMPTY PAGE** (founder, 2026-09-18, phone: after subscribing the screen said *"Your plan details will appear here"*). ★ Two causes. The wizard called `invalidateEntitlement()`, which DROPPED the store's copy, so Subscription & billing lost every existing subscription and invoice until the 20-second poll; it now calls `syncEntitlement()` (twice — a poll already in flight hands back the pre-purchase answer), so what she had stays on screen. And nothing told the screen an invoice was coming: new `lib/purchase.js` holds the scopes just bought; the screen re-reads `/invoices` on FOCUS (it is under the wizard, so there is no remount), polls every 2 s for up to 30 s, and draws the preparing card's own bar (`sc_prep*`, pine) — "Preparing your invoice…" — in the Invoice row of each new scope, with a card for any scope the entitlement copy has not caught up with. The trial status card and its Subscribe button stand down while a purchase settles. Cleared on sign-out. Web unchanged (its SubscribeFlow re-reads on done). Unwalked. |
| **billing·order** | **NEWEST PURCHASE ON TOP** (founder, 2026-09-18: *"latest top, oldest last"*). `subsFromEntitlement(ent, invoices)` now orders by WHEN SHE BOUGHT: the issue time of the newest invoice listing the scope, falling back to one year before its validity (a manual grant has no invoice). Until-date alone had put a scope carrying only the account-wide date in the wrong place. Web Settings and the phone both pass their invoices (+1 test, 213 green). On the phone the just-bought cards now render ABOVE the rest. ★ **AND A WHITE-SCREEN BUG IN `billing·settling` WAS CAUGHT BEFORE ANY HANDSET SAW IT**: `planCard` read `purchasing` above its declaration — a TDZ ReferenceError on first paint of Subscription & billing. The second such bug in two days (`tourNow`, 2026-09-17), so it now has a checker: **`mobile/check-tdz.mjs`**, in `npm run check` — a `const`/`let` read before its declaration in the same function; nested handlers and effect bodies are allowed, effect DEPS arrays are not. Proved on a probe and on the shipped file; the whole of `mobile/` and `web/app/` is clean. |
| **trial·ledger** | **A FREE TRIAL IS ONCE PER MOBILE NUMBER, EVEN ACROSS AN ERASURE** (founder, 2026-09-18). Delete → sign up again used to hand out three more free chapters for ever. ★ **Her profile is still forgotten; one line is not**: `aruvi_core/adapters/trial_ledger_file.py` keeps an HMAC of the number under `ARUVI_TRIAL_LEDGER_KEY` (never the number — a plain hash of ~10^10 mobiles is reversible by brute force), the chapters it used (a paid account counts as spent) and a forget-after date **24 months** out; `trial_ledger/{hash}.json` sits outside the erase walk (the `erasure_log/` precedent); an entry never shrinks. Wired: `data_rights_erase` writes it BEFORE the entitlement is destroyed; `_entitlement_of`'s JIT trial seeds that many `_earlier/N` placeholders (never a real chapter key, so no free re-serves); `/onboarding/verified` returns `trial_remaining`, and both front doors send a number with 0 left to Subscribe with "This mobile number has already used its free trial. Subscribe to keep using Meyy." instead of into first run to meet a paywall. **Disclosed** in Privacy Notice **v0.3** (§2 + a new §7 row), User Agreement **v0.5** (§C + §G) and `_KEPT` (7 rows now; `test_privacy_notice` pins them) — every existing teacher sees the notice bar once and re-ticks the agreement at her next purchase. ⚠️ **For counsel**: retention for a business interest after an erasure request is not obviously within DPDP §8(7); `docs/legal/privacy_policy_considerations.md §8`. `render.yaml` generates the key once (`generateValue`) — never rotate it. `tests/test_trial_ledger.py` (7, sabotage-verified). ⓶ **No archiving an attached lesson** (founder): the UI already hid the control; `POST /plan-archive` now refuses with 409 (`tests/test_archive_attached.py`), and tour step 5 is SKIPPED on both surfaces when the tour's lesson still reads as attached (4→6, 6→4) — the web via `onTourArchivable`, the phone via `noteTourArchivable`. ★ Found on the way: the plan listing's `bound_files` read `st.chapter` through `getattr` on what the store keeps as plain dicts, so it was ALWAYS empty and a bound-but-unprepared plan shipped no unit count for its rail — fixed. And `test_section_history` was red since the shared-package move (it read the web's re-export for `tsOf`) — pointed at `packages/shared`. ⓷ SMS (DLT): unchanged, parked on registration. ⓸ **No subject removal** (founder): she empties a paid subject's classes instead; app. 02 row 69 closed, the dead confirm joins the post-walk deletion list. |
| **measure·390** | **THE OWED 390px MEASUREMENTS, TAKEN ON THE RUNNING WEB** (2026-09-18, Claude's browser pane — Chrome was unreachable; probe elements carrying the real class names, read with `getComputedStyle` at a 390×844 viewport). ⓵ The `7·clusters` keys: `sc_hist`, `ch_row`, `ch_meta`, `ch_name`, `ch_no`, `ch_rail` agree as derived; three line heights corrected — `ap_loading` 19 → 20.15, `ch_pill` 12 → 13.18, `uv_va_src` 17.28 → 18.6. The "derived, not measured" caveat on `7·clusters` is retired. ⓶ `.uv-va-table` (06·69b) measured and `AidTable` rebuilt as the web's ruled table. ⓷ Web tidying: `saveEditNums` onto `setGradeNumbers`/`setGradeBudget` (02·92), `writePointer` onto `setUnitPointer` (06·6). The web page recompiled clean (only the known theme-attribute hydration warning). ⚠️ **NOT done, deliberately: `page.jsx` onto the shared readiness store.** It is not the no-visible-change tidy it was listed as — the store paints from the device copy on a network error where the web today clears and waits, which changes what a teacher sees on a flaky connection; it wants its own decision and walk. |
| **web·readiness** | **THE WEB KEEPS A COPY OF HER PROFILE, LIKE THE PHONE** (founder, 2026-09-18: *"web should mimic phone"* — her plans are exportable anyway, so a device copy gives nothing away). The bug that settled it: on a failed `/readiness` the web set `ready = false` and a set-up teacher whose Wi-Fi blinked on opening Meyy met the **first-run welcome screen**. `page.jsx` now paints from `cachedReadiness()`, revalidates behind it (`fetchReadiness({force:true})`, reading the STORE afterwards so "she has no profile" and "the read failed" stay distinct), keeps signing out on a 401, and writes every change through with `adoptReadiness`. Sign-out already cleared the store (`clearTeacherCaches`). Static-verified only — the local web points at the Supabase-auth API and this session does not sign in with an OTP. **Walk:** sign in on the web, reload (instant, no flash), then stop the API (or go offline) and reload — her classes should still show, not the welcome screen; sign out and in as another number — none of the first teacher's classes. |
| **text·size** | **THE IPHONE'S TEXT SIZE, CAPPED — AND A MEYY CHOICE THAT OVERRIDES IT** (founder, 2026-09-18, reversing the 2026-09-13 "ignore the slider" decision in part). `components/Text.jsx`: *Match iPhone* (default) honours Control Centre › Text Size up to 1.2×; *Standard* / *Large* 1.1× / *Larger* 1.2× turn OS scaling off and scale the style themselves, so one ceiling covers both paths. New `components/TextSizeToggle.jsx` on a "Text size" card under Appearance (a word, not a glyph — ThemeToggle's own note says a cycling glyph is hard to discover without hover); preference `aruvi-text-size` in `ThemeContext`, survives sign-out. Every Text/TextInput already imports from `components/Text.jsx` (grep-checked: none bypasses it). Phone-only by nature — the web has browser zoom. **Walk owed at Larger AND at the iPhone's largest size**: bottom bar labels ("MY LESSONS" is the widest), section cards, lesson unit tabs, lesson-material tables, the top bar, the set-up window rows. |
| **offline·web** | **OFFLINE, FROM THE FOUNDER'S OWN WALK** (2026-09-18). ⓵ Wi-Fi off → the web sat on "Connecting to the Meyy engine…": `subject` came only from GET /subjects (the catalogue); it now falls back to the first subject in her saved profile. ⓶ Opening a lesson offline threw an unhandled "Failed to fetch": the plan LISTINGS were cached but the view never was. New `fetchPlanView` / `cachedPlanView` in `@aruvi/shared/plans` — network first, the device copy on failure, a 404 never papered over, the 12 most recently opened kept (LRU, quota-safe), keys under `aruvi_plans_` so sign-out sweeps them (+5 tests, 218 green). Used by web MyPlans, MyLessonPlans, SectionProgress and the phone's lesson route; a lesson never opened on the device says so instead of crashing. ⓷ Text size follow-up: "Device Default" on two lines at 8.5px so the card keeps its height; card copy "Match your Device default or choose between standard, large and larger."; the bottom bar's My Classes / My Lessons labels use a new `fixed` prop on `Text` and never scale. All unwalked. |
| **preview·word** | **WORD IS SHOWN BEFORE IT IS SENT TOO** (founder, 2026-09-18: the Year Plan export *"again does not display the report but invokes apps"*). `canPreview` now admits `.docx` on iOS — WKWebView draws Office files loaded from a file URL through Quick Look, read-only — so the Year Plan arrow opens `/preview` (label "Year plan · {subject}") and the arrow there sends the editable Word file. Same path the invoice, the lesson PDF and the data export already take, so the lesson report's Word option and Your data's Word download now preview as well. Android unchanged (its WebView renders neither). ⚠️ The old note said no WebView renders .docx; **handset-verify** — if one ever draws blank, the preview's "Tap the arrow above to send or save it" is the way out. |
| **fr·subjects** | **"LOADING SUBJECTS…" THAT NEVER ENDS** (founder, 2026-09-17: *"the web app is stuck in loading subjects for 9000000001"*). ★ **TWO STATES WHERE THERE ARE THREE.** `catch(() => setSubjects([]))` plus a render testing `length === 0` cannot tell *has not arrived yet* from *will never arrive*, so both a failed `/subjects` AND a legitimately empty list sat under the word **Loading…** for ever — on the FIRST STEP of first run, with no error and nothing to press. Now `subjLoad` is `""` · `"ok"` · `"fail"`, with a **Try again** on failure. Fixed on BOTH surfaces (§0). ⚠️ **AND THE EMPTY LIST IS THE LIKELIER CAUSE FOR THIS ACCOUNT, which is why it gets its own sentence rather than a retry.** `/subjects` is an UNGATED catalogue read — no auth, no entitlement — so it cannot fail for one teacher and not another; but `frPaidScopes` filters the wheel to the scopes she BOUGHT, and `ARUVI_ENTITLEMENT_ENFORCED=1` on Render (founder, 2026-09-11) means that filter is LIVE in production. A grant whose scope subject does not match a catalogue slug — `social_science` for `social_sciences`, a display name for a slug, a stage that does not exist — empties the wheel permanently and silently. ⚠️ **`aruvi-scripts/entitlement.py` TAKES `--scopes` VERBATIM and validates nothing against the real catalogue**, so a founder typo at grant time becomes a teacher stuck on step 1 with no diagnosis available to her or to support. Validating there would turn a permanent stall into an error at the keyboard — NOT done here, because it is a server-side tool outside Track D and deserves its own change | *this commit* | ✅ 71 files parse and resolve clean, 203 shared tests green. 🟡 **Unwalked** — and the 9000000001 diagnosis is UNCONFIRMED: the API host is not on this sandbox's egress allowlist, so its entitlement could not be read from here. `python3 aruvi-scripts/entitlement.py status 9000000001` against the production state settles it in one line |
| **8b·walk1** | **THE FIRST HANDSET WALK OF THE TOUR — twelve symptoms, seven causes, and two that need a decision** (founder, 2026-09-17). ★ **ANCHORS.** ⓵ `lesson-first` was DECLARED and never ATTACHED — cards 3 and 6 had no ring, and a tip with no rect falls to the bottom of the screen, which is what *"stuck at the bottom"* is. ⓶ `section-add` was attached only to the EMPTY card's "+", right for step 8 and impossible for step 14, which happens after the demo attach when the card is bound and complete and renders the other branch. Both branches now carry it; they are mutually exclusive. ⓷ **`collapsable={false}` is load-bearing** — RN flattens a style-only View and a flattened node has no `measureInWindow`, so the ref alone would not have been enough. ★ **TIMING WAS THE OTHER HALF OF "stuck at the bottom".** My Lessons paints *Loading plans…* until the server answers, so on a cold Render instance the card does not exist for a second or more after the step changes — and the overlay's ONE 220ms retry had long since given up. Registration is now an EVENT (`onAnchorRegistered`), so the ring lands the moment its target exists; the timed retry stays as a bounded backstop (4 tries, ~2s) for a node that is registered but still measuring as a zero box. Plus ONE confirming re-measure per step at 450ms, because a measurement taken while a screen is still settling answers with a position that is real and already stale — the found/not-found retry cannot catch that, it stops the moment there IS a ring, right or wrong. ★ **NAVIGATION.** There was NO move for step 6, so the phone narrated a preview it had not opened and step 7's anchor could not exist; `openTourLesson({preview:true})` opens it with no section, which is what makes it a preview and makes it claim `preview-root`. ★ **AND STEPS 11-13 SHOWED THE CHAPTER MAP ON BOTH SURFACES** — the tell that it was the *landing rule* (no progress → the org page, founder 2026-09-14) and not either port. The tour now says which it wants (`tourUnit`); nothing else passes it. ★ **THE ONE-SHOT WAS BEING EATEN.** The tour walks back to My Classes at 7→8, that focus SPENT the set-up check, so it popped mid-tour AND was gone by the end — one bug, both halves of the founder's report. Held while a tour is running. ★ **AND THE OFFER SPRANG BACK.** `cachedAccount()` is a cache the tour-offered POST does not rewrite, so the instant `step` returned to 0 every term was true again — and taking it again re-ran the demo over her real section. `tourRanHere()` is the missing half; ⚠️ NOT `offeredHere()`, which flips when the nudge becomes ELIGIBLE and would hide it while she is looking at it. ★ **AND THE UNBIND WAS TOO BROAD**: every step 1-9 satisfies `<= 9`, so re-entering the tour stripped a binding the teacher already had, on step 1, before she had read a word (*"going back to where we began"*). `tourBoundRef` — undo only what THIS run attached. Both surfaces. ⚠️ **TWO SYMPTOMS ARE NOT FIXED AND MUST NOT BE GUESSED AT.** ⓵ **Steps 9 and 15 sit under a `Modal`** — RN presents one in its own native window, so the overlay in the shell cannot draw above the attach sheet: no hand on the row, and Next unreachable, which is why pressing on left the ring at the popup's size (what she pressed was the ROW, which attaches and closes the sheet while the step never moved). Fixing it means either passing the overlay INTO the sheet or making the sheet non-modal during the tour — an architecture choice, and one that must be walked. ⓶ **No archive control on an ATTACHED plan**, so step 5 has nothing to ring on a profile whose only lesson is attached — a product question (should the tour skip the step, or should an attached plan be archivable?), not a patch ★ **AND THE OFFER NOW WAITS FOR THE LESSON** (founder, 2026-09-17: it *"should pop up immediately after the completion of the lesson generation and not at the same time"*). First run hands her to My Classes with the generation STILL RUNNING, so the nudge was landing on top of the wait she is watching — two things asking for the same attention, and the one she cares about is the chapter. The completion edge already existed on both surfaces (`preparing` going false / `preparingCard` going null) because it is what redraws the card; the offer simply rides it. It also closes a latent hole: `tourTarget` needs a PREPARED plan to point at, so an offer taken mid-generation could open the tour with nothing to demonstrate on. ⚠️ On the web this meant HOISTING `preparingCard`'s declaration above the offer — it is read during render, and a const read above its declaration is a TDZ crash on first paint that no parse or lint sees. | *this commit* | ✅ 70 mobile + 6 web files parse and resolve clean, parity exits 0, 203 shared tests green. 🟡 **Needs a second walk** — ten fixes against a report from one, and only the handset can say which landed |
| **front·subscribe** | **THE PHONE'S CHOOSE SCREEN OFFERED ONLY ONE DOOR** (founder, 2026-09-17, after a delete-and-rejoin: *"only free to try shows and subscribe does not"*). App. 03 rows **7 · 24 · 40 · 42**, owed since 2026-09-15 — that family now has none left. ★ **THE CODE WAS NOT THE PROBLEM; THE STALE NOTE WAS.** `login.jsx`'s header said the beta ran on manual grants with "no purchase screen in the app", which stopped being true on 2026-09-16 when Q11 shipped the wizard — so a card was kept off a screen whose destination already existed, for a day, by a reason nobody re-read. **A note that outlives its reason is worse than no note: it is an instruction.** ★ **ONE WIZARD, TWO DOORS.** `app/(app)/subscribe.jsx` was 525 lines of screen; it is now `components/SubscribeWizard.jsx` plus two thin routes, which is the shape the web has had from the start (`SubscribeFlow` serves both). The doors differ in EXACTLY three props — where Done goes, where Cancel goes, and `trialFork` — so the fork is enforced by what is passed rather than by what is absent. ★ **THE FRONT DOOR SITS OUTSIDE `(app)`** because she is authenticated by then (the OTP is verified and `/onboarding/verified` has made the account — every call in the wizard needs that) but has NOT been through first run, and `(app)`'s gate would bounce her to `/first-run` before the wizard drew a frame. What the front door skips is the APP, not the sign-in. ⚠️ **Its file name is load-bearing:** `(app)` is a route GROUP and adds no path segment, so `app/subscribe.jsx` would have been a SECOND route claiming `/subscribe` — hence `/front-subscribe`, and the header says so, so it is not tidied back. ★ `mode` is read in ONE place, on the CREATE path only: a returning teacher never saw the choose screen, so her default `mode` must not send her to a purchase she did not ask for ★ **AND THE CHOOSE SCREEN WAS NEVER UNDER THE PARITY CHECKER** (founder, 2026-09-17, the same walk: *"align color of 'subscribe' with that of web app... ensure everything else in that page is aligned"*). It drew from a LOCAL `StyleSheet` with `type.*` faces, so the checker had nothing to compare and three things had drifted unseen: ⓵ **the two headings are different colours** — `.ob-plan-hd` is PINE, `.ob-plan-sub2 .ob-plan-hd` is CLAY, which is the whole visual difference between the offers, and the phone painted both `ink`; ⓶ **`on` does not tint the fill** — the web keeps `card-bg` and adds a pine border plus a 1px ring, where the phone washed the chosen card in `tint_pine` and thereby made the UNCHOSEN one read as disabled, on a screen whose entire job is offering a choice; ⓷ **`.ob-h2` carries a RULE above it** (border-top + 16px) that separates the pitch from the choice — the phone had a bare gap. Now seven keys in `theme/web.js` (`ob_h2`, `ob_plan`, `ob_plan_on`, `ob_plan_hd`, `ob_plan_hd_sub2`, `ob_plan_sub`, `ob_plan_points`) and the dead local style deleted, so the next drift fails a check instead of waiting for a walk. ⚠️ `ob_plan_hd` is SEMIBOLD although its CSS rule declares no weight: the markup is `<span class="ob-plan-hd"><b>…</b></span>`, the checker cannot see a `<b>`, and what the web RENDERS is what the phone has to match. **A screen drawing from a local StyleSheet is a screen outside the parity contract — that, not the colour, is the finding.** | *this commit* | ✅ 72 files parse and resolve clean, parity exits 0 with all eleven new keys (`ob_offer_*` and `ob_plan*`) matching their CSS, 203 shared tests green. 🟡 **Unwalked** — and the walk that matters is the WHOLE front door: choose Subscribe, verify, take the Trial fork (lands in the app), then do it again and decline it (lands in the wizard) |
| **tour·offer2** | **THE OFFER WAS FIXED ON THE SCREEN FIRST RUN DOES NOT FINISH ON** (founder, 2026-09-17: *"on expo, the tour pop up comes before the lesson is complete even now. Web app is fine"*). ★ **THE WEB IS FINE BECAUSE IT HAS ONE OFFER** — `page.jsx`, above the tab, so its single `tourOnOffer` IS the rule. The phone has no shell above its routes, so My Classes and My Lessons each render their own nudge and each had hand-rolled the terms: My Classes carried all four, **My Lessons carried ONE** (`tourFit`). And first run `replace`s to **/lessons** — which is where the wait belongs, because that is where the lesson will appear — so every fix to the offer had been landing on the screen she was not looking at. The `preparing` gate, the `tour_offered_at` check and `tourRanHere()` were all absent from the one that mattered. ★ **`tourOfferOpen` IS NOW THE ONLY COPY** and both screens call it; the terms and their reasons moved to it, so a future fix cannot land on one screen again. ⚠️ `preparing` is PASSED, never read inside: the two screens own opposite descriptors by design — My Classes' has a section, My Lessons' does not — and only each screen knows which is its own. **The bug was not the gate; it was that there were two gates and only one was ever fixed.** | *this commit* | ✅ 72 files parse and resolve clean, parity exits 0, 203 shared tests green; grep confirms two `<TourOffer>` mounts and two `tourOfferOpen` callers, one each. 🟡 **Unwalked** — the walk is first run end to end: the nudge must NOT appear while the bar is running on My Lessons, and must appear as the finished card lands |
| **get·retry** | **"OK AFTER 3 OR 4 PRESSES" IS THE SHAPE OF A TRANSIENT** (founder, 2026-09-17, screenshot from first run on …02: *"why does this come in first time gen for profile ****02. it seems to be ok after 3 or 4 presses"*). The error itself was the **fr·subjects** work behaving correctly — a failure that used to sit forever under the word *Loading…* now says so and offers Try again. What it revealed is that the client had **no retry at all**: one bare `fetch`, no timeout, and the teacher pressing the link WAS the retry loop — on the first screen a brand-new account ever sees. ★ `getJSON` now retries itself: 3 attempts, 15s deadline each, 500ms then 1500ms backoff. ⚠️ **ONLY GET, AND ONLY TRANSIENT STATUSES.** A GET is idempotent so repeating one is safe; `postJSON` is deliberately NOT given this, because repeating a checkout or a generation is not. And **a clean 4xx is an ANSWER, not a failure** — `/onboarding/known` answers 404 for a number we do not hold and `/plans/…/view` answers 404 for a deleted plan, and callers READ those codes; retrying them would turn a correct instant answer into a three-second wait for the same one. Only a thrown fetch and 408 · 429 · 502 · 503 · 504 come back. ⚠️ **Each attempt needs its own deadline or the retry never happens** — a hung socket blocks the first attempt for as long as the platform allows, which on a mobile network is a minute or more. ★ **AND THE REASON NOW GOES TO THE CONSOLE** on both surfaces: the teacher's sentence stays *"Couldn't load the subject list"*, but a handset report can finally say WHICH failure it was. ⚠️ **THE CAUSE IS STILL UNCONFIRMED** — the API host is not on this sandbox's egress allowlist, so it could not be reproduced from here; `render.yaml` says `plan: starter`, which does not spin down, so a first-TLS-handshake or radio-reattach stall is as likely as a cold backend. The retry absorbs all three | *this commit* | ✅ **Six new tests** (209 total, all green) fixing both halves of the rule — that a 503 and a thrown fetch come back, and that a 404, a 402 and a plain 500 cost exactly one round trip. 72 files parse and resolve clean, parity exits 0. 🟡 **Unwalked** — and if it recurs, the Expo console now names the failure |
| **8b·walk2** | **SECOND HANDSET WALK — and the biggest finding was MY OWN FIX FROM THE FIRST** (founder, 2026-09-17, ten items). ★ **CARDS 4, 5, 8, 9 ON BOTH SURFACES HAD ONE CAUSE, and the founder named it himself:** *"The lesson card shows lesson already attached! and that is why archive box does not show"*. The walk-1 fix — *only undo what the tour itself attached* — was the wrong rule. **The demo REQUIRES an unbound section for steps 1-9**: the archive control hides on an attached plan (step 5), the section card draws its "+" only when nothing is bound (step 8), and the picker lists a chapter only when it is not the bound one (step 9). **First run ATTACHES what it generates**, so the tour's own audience ALWAYS arrives bound — which made a guard that "never touches an existing binding" a guard that breaks four steps out of twenty. ⚠️ But the harm it was written against was real too (Skip at step 5 left her section stripped). **So the tour BORROWS**: what was bound at the start is remembered, 1-9 unbind, 10 binds the demo plan, and a tour that ENDS with the section empty gives back what it took. A completed tour ends bound to the demo plan — which for a new teacher IS what she had — so nothing is restored and nothing is lost either way. **The lesson: a guard written from one symptom, without walking the steps it governs, traded four broken cards for one.** ★ **AND STEPS 11-13 OPENED A LESSON WITH A MALFORMED KEY.** `openTourLesson` passed `tg.sectionKey` where `lesson.jsx` builds the key ITSELF (`${subject}_${grade}_${section}`), so the tour asked for `science_ix_science_ix_9A`. The screen still counted as tracking and looked right — it claimed `lesson-root` — while every read behind it answered from nothing and every write went somewhere that will never be read again. My Classes' own `openAttached` has always passed the TAG; this was the one caller that did not. ★ **Step 14 now sits BELOW the section card** on both surfaces (founder), replacing a 10%-of-viewport lift that put the tip nowhere near the "+" it describes. ★ **And the set-up window's *"see your full teaching profile"* link was an EMPTY handler** whose note read *"the full accordion arrives with Settings, step 6"* — Settings arrived in 6b. **Second time in one day a placeholder outlived its reason and went on being obeyed** (the login header kept the Subscribe card off a screen that already existed). **A note that states a dependency must name the thing that lifts it, and be deleted the day it lands.** ⚠️ Closing the window first is a phone-only divergence, named: the web leaves it standing because its profile replaces the view; here it is a `Modal` that floats above every route, so navigating under it delivers a profile she cannot see. ⚠️ **STILL OPEN, and NOT guessed at a fourth time: steps 9 and 15 under the `Modal`** — reported three walks running. RN presents a Modal in its own native window, so the shell's overlay cannot draw above the attach sheet: no hand on the row, and Next unreachable. It needs a decision (pass the overlay into the sheet · lift the tour's navigation out of `_layout` so any screen can render it · make the sheet non-modal during the tour) and a walk, not a third guess. ⚠️ **Step 7 still reported failing and the cause is unknown** — `openTourLesson` now SAYS when no target was published, so the next walk separates "the move never fired" from "the move fired and the anchor missed" | *this commit* | ✅ 72 files parse and resolve clean, parity exits 0, 209 shared tests green. 🟡 **Third walk owed** |
| **tour·nav** | **THE TOUR'S NAVIGATION LEFT THE SHELL** (founder chose it, 2026-09-17, after the same symptom was reported on three walks: at steps 9 and 15 *"pressing next shows the erroneous window"*). ★ **THE CAUSE WAS STRUCTURAL, WHICH IS WHY TWO GUESSES HAD FAILED.** The attach picker is a React Native `Modal` — presented in its OWN NATIVE WINDOW above the whole app — so an overlay rendered in `(app)/_layout.jsx` could never draw over it, at any z-index. She saw the previous step's ring, could not reach Next, and the only thing she COULD press was a lesson row: that attaches and closes the sheet while the step never moves. **The "erroneous window" in the report was the step that never advanced.** ★ **THE FIX IS NOT TO FIGHT THE MODAL BUT TO LET THE SHEET DRAW THE TOUR** — and it can only do that if advancing the tour is something ANY screen may call. So `tourNext` · `tourBack` · `tourSkip`, the two move tables and `openTourLesson` moved from the shell into `lib/tour.js`, using expo-router's module-level `router`. `TOUR_TOTAL` moved with them (importing the component from the module would be a cycle, and the count is a fact about the tour, not about the overlay that draws it). ★ **EXACTLY ONE PLACE DRAWS IT AT A TIME** (`setTourOverlayHost` / `useTourOverlayHost`): the shell for nineteen steps, the sheet while it is up, released on unmount so a sheet closed mid-tour hands the job straight back. Two overlays would double the scrim and stack two tips. ★ **AND ASK MEYY IS NOW OPENED THROUGH ITS OWN STORE** rather than the shell's local state — which closes a small existing hole where `isAskOpen()` disagreed with what was on screen, because the tour set the local copy only. ⚠️ **THE REFACTOR EXPOSED A REAL BUG IT DID NOT CAUSE:** the demo effect returns early once the tour has ended, so its own `setAttachFor(null)` never ran on the way out — Skip at step 9 or 15 left the picker standing with the overlay gone. The web has always closed it (`MyPlans`'s tour-ended effect); the phone had never noticed, because before this the sheet could not usefully be on screen at those steps at all. **A structural fix earns its keep twice: once for the symptom, once for what it makes visible.** | *this commit* | ✅ 72 files parse and resolve clean; two `<GuidedTour>` mounts and exactly one host; all 19 anchors wired; the phone's and the web's step tables agree on every anchor AND placement (checked mechanically); parity exits 0; 209 shared tests green. 🟡 **Unwalked** — the walk is steps 8→10 and 14→16: the hand must land on the lesson ROW, Next must advance, and Skip from inside the picker must close it |
| **8b·walk3** | **THIRD WALK — and the phone's tour was orchestrated by a screen that UNMOUNTS** (founder, 2026-09-17). ★ **CARDS 4 AND 5 ON EXPO HAD A CAUSE I HAD NOT LOOKED FOR:** the bind/unbind lived in `(app)/index.jsx`, and **My Classes is a ROUTE that unmounts** the moment the tour walks to My Lessons for steps 3-6. The web runs the same logic from `MyPlans`, which is a TAB and stays mounted the whole way — so the phone's unbind never ran, the lesson card still read as attached, and the archive control (hidden on an attached plan) never existed for the ring to find. **Two walks were spent fixing the RULE while the code enforcing it was not running.** Moved into `lib/tour.js`, driven by `setTourStep`, same reasoning as the navigation move: what the tour must DO at a step belongs to the tour. ★ **AND THE TARGET IS NOW DERIVED, NOT WAITED FOR.** It was published by My Classes — fine while the tour opened there, and fatal once it does not: the offer is on My Lessons, first run leaves her on My Lessons, and steps 1-2 now stand there too, so My Classes may never mount at all. That is what the *"no target published"* diagnostic was telling us, and it is **step 7's long-unexplained failure**. `deriveTarget` reads the readiness and plans caches, both already warm; a published target still wins. ★ **STEPS 1 AND 2 NOW STAND ON MY LESSONS** on both surfaces (founder: the web showed *"My Classes background but it should be in My Lessons only"*) — `startTour` opened with `goClasses()`; it now opens where she already is. The moves either side of step 3 went with it. ★ **AND STEP 7 SHOWS THE ORG PAGE AGAIN** — I over-applied `tour: "1"` (which forces the unit for 11-13) to the PREVIEW, where the web lands on the chapter map. One flag, two journeys; it now rides only the one that asked. ⚠️ **AND A RUNTIME BUG NO CHECK COULD SEE:** `lib/tour.js` imported `gradeSlug` from `@aruvi/shared/format`, which **did not export it** — it was copied privately into FOUR screens, two per surface. An undefined import is a TypeError at the call, invisible to a parse and to `check-scope.mjs` (the binding resolves; the VALUE is undefined). Lifted beside `subjectSlug`, where its twin always lived, and all four copies deleted. **A helper half the app agrees on, defined nowhere shared, is a trap waiting for the fifth caller.** | *this commit* | ✅ 72 mobile + 3 web files parse and resolve clean, parity exits 0, 209 shared tests green, both step tables agree on all 19 anchors and placements. 🟡 **Unwalked** — cards 12 and 13 were not touched this round: the section-key fix (walk 2) and the org-page fix should have settled them, and a fourth walk decides |
| **8b·walk4** | **FOURTH WALK — and three of the four had ONE cause I had never looked for: an anchor on a WRAPPER around an absolutely-positioned child** (founder, 2026-09-17, having reported cards 12 and 13 and the web's archive *four times each*). ⚠️ **IN REACT NATIVE THAT IS FATAL TWICE OVER.** An absolute child positions against its PARENT, so the wrapper becomes its frame of reference and moves it on screen — which is the founder's *"the export icon is not placed properly"*, the same bug's other face. And a View whose only child is absolute contributes NOTHING to layout, so it measures as a **zero box**, which `measureAnchor` correctly treats as *absent* — the ring has nothing to draw and the tip falls to its no-rect fallback at the foot of the screen, which is *"card 4 … too close to bottom rather than the lesson card"*. **Nothing fails loudly. That is why it survived four reports.** ⓵ `lesson-report` was on a `<View>` wrapping `ReportButton` (`sc_report` is absolute) — the name now goes INTO the button and rides its own Pressable, which is the shape the web already had (`dataTour` as a prop). ⓶ `phase-bookmark` was on a `<View>` wrapping `PhaseBookmark`, whose root is `position: "absolute"` — card 12, four times. The wrapper is gone; the bookmark takes an `anchorRef`. ★ **AND `check-anchors.mjs` NOW FINDS THE SHAPE** — `npm run check` — because a mistake made twice in one file family will be made a third time. It reports candidates rather than failures (it cannot prove a child is absolute); the two it flags today are `Round`, which is 30×30 and in flow. ★ **AND THE WEB'S ARCHIVE HAD THE WALK-3 BUG ALL ALONG, because my premise was wrong.** I wrote that `MyPlans` "is a TAB and stays mounted". It is not: `page.jsx` renders `MyLessonPlans` **INSTEAD of** `MyPlans` in a ternary chain when `editFlow === "lessonplans"`, so it unmounts for steps 3-6 exactly as My Classes does on the phone — and the unbind never ran, so the plan read as attached and `.mlp2-iconbtn.archive` (absent on an attached plan) was never in the DOM. **I fixed the phone three times on a premise about the web I never checked.** The binding now lives in `page.jsx`, which is the shell. | *this commit* | ✅ 74 files parse and resolve clean, anchors swept, parity exits 0, 209 shared tests green. 🟡 **Unwalked.** Card 13 was NOT changed: its anchor sits on a styled View with real size, so the wrapper trap does not explain it, and the walk-2 section-key fix plus the walk-3 org-page fix may already have. If it survives a fifth walk the next suspect is that `mark-complete` sits BELOW THE FOLD with nothing scrolling to it |
| **8b·card13** | **CARD 13 ALONE — and the answer was written in the WEB'S OWN COMMENT** (founder, 2026-09-17: *"what is the problem with 13 alone on expo now"*). ★ **THE PHONE PORTED HALF OF THE WEB'S SCROLL HANDLING.** The web's overlay does two things with a scroller: it PINS to the top for the two steps that ask (`scrollTop`, 7 and 11), and it brings an off-screen target INTO VIEW for every other step — whose comment reads, word for word, *"Bring an off-screen target into view (e.g. Mark complete below the fold)"*. **The phone ported the pin and never ported the other half.** ★ **SO NOTHING WAS BROKEN, WHICH IS WHY FOUR ANCHOR FIXES CHANGED NOTHING.** `mark-complete` sits at the foot of a unit's lesson panel; the moment the phases run past one screenful it is below the fold. The anchor registered, the ring measured, the ring was DRAWN — off screen, where she could not see it. And `place: "above"` on a box below the fold computes `vh - box.top` NEGATIVE, so `Math.max(12, …)` dropped the tip to the foot of the screen. Everything worked and nothing was visible. **The lesson: when the ring is absent the anchor is the suspect; when the ring is DRAWN and unseen, the viewport is.** ★ The scroller is now the pair the web has: `{ top, by }`. `by` takes a DELTA because the overlay measures in WINDOW coordinates and only the scroller knows its own offset; `LessonView` tracks it with `onScroll`. Bounded exactly like the web's — 5 tries, 400ms apart, and only while the ring is outside a 96px margin — because a scroller that keeps re-centring fights a teacher reading under the box, which is the complaint the pin's own two-try limit exists to prevent. ⚠️ It applies to EVERY step that does not pin, so any future anchor below the fold is covered by construction rather than by a per-step flag | *this commit* | ✅ 72 files parse and resolve clean, anchors swept, parity exits 0, 209 shared tests green. 🟡 **Unwalked** — card 13 should now scroll Mark complete to the middle of the screen before ringing it |
| **8b·tour** | **THE GUIDED TOUR IS ON THE PHONE — twenty steps, nineteen anchors, and the machinery rebuilt rather than ported** (app. 01 rows 59-61, 96-118). ★ **THE COPY IS A TRANSCRIPTION**: every title and body is the web's word for word, and the anchors keep the web's own `data-tour` STRINGS — `lib/tour.js` says why that is not tidiness. One step table serves both surfaces, and the moment the phone invents its own names the two can drift a step apart without anything failing loudly. ★ **THREE THINGS HAD TO BE REBUILT.** ⓵ `document.querySelector` becomes a REGISTRY: every anchor registers its ref, measured on demand with `measureInWindow`. ⚠️ That call is asynchronous and can answer after unmount, so every measurement checks it is still the step on screen — without it a fast Next lands the previous step's ring on the next step's screen, which reads as "jumping" and is near-impossible to reproduce deliberately. ⓶ **The 200 ms POLL is gone**: measurement happens on step change, on anchor layout and on rotation, because anything that moves an anchor does one of those three. An interval is a battery cost with no payer. ⓷ **The ring is FOUR VIEWS, not a `box-shadow`** — and that substitution is a GIFT rather than a cost: four real rectangles around the rect give hit-blocking for free and leave the hole genuinely tappable, where the web needs `pointer-events` juggling on a stack of layers. ★ **AND THE ANCHORS ARE CONDITIONAL, which the port had to learn the hard way**: only the tour's own card claims `lesson-first`/`section-add`/`section-card-target`, and only on the step that rings it — put a name on every card and `measureAnchor` returns whichever mounted first, which is rarely the one she is looking at. ★ `finishTour` does NOT raise the set-up check window (founder, 2026-09-17, closing Q9 — see `lib/firstRun.js`). ⚠️ **A SCOPE BUG BABEL CANNOT SEE**: `unitTabsTourRef` was declared in one component and used in `PreviewUnit` — two components, one file, parse-clean, and a WHITE LESSON SCREEN at runtime. It was found, moved, and found AGAIN in the same file two sessions later, which is the argument for a tool rather than a note: `mobile/check-scope.mjs` now resolves every referenced identifier against its real scope chain. ⚠️ Its first version consulted `scope.hasGlobal` and reported the live bug as CLEAN — babel counts every unresolved reference as a "global", so asking it waves through exactly what the script is for. It is proved against a two-line file with a known stray reference before its green is believed, and that proof is written into its header. An anchor ref belongs to the component that RENDERS it | *this commit* | ✅ **COMPLETE, NOT WALKED.** All three gaps closed this session. ⓵ **The DEMO IS REAL**: `tourTarget` picks the first prepared plan, the 9→10 boundary calls `bindSectionChapter` and 13→14 shows the unit done — and the completion is a DRAWING flag (`tourDemoDone`, mirroring the web's `tourStep === 14 || 15`), never a write, so a tour she abandons leaves no phantom progress. The unbind on the way BACK is what makes it repeatable. ⓶ **The pin is wired AND it survives the ordering**: `PreviewUnit` registers the scroller, and because steps 11 and 13 navigate and advance in the SAME TICK, `pinTourScroll` holds a pin for the scroller that is about to register — spent once, cleared by the next step and by `endTour`, so it can never fire into a screen she opened herself a minute later. ⓷ **The offer is on My Lessons too**, with the one word that differs by surface carried on a `here` prop. Also fixed on the way through: the org page had no root anchor, so steps 7 and 11 had nothing to ring on exactly the plan a NEW teacher is shown — a chapter with no progress opens on the map, not on a unit. ✅ **Verified:** 70 files parse clean and resolve clean (`check-scope.mjs`), all 19 anchors wired — `handAnchor` is a third place a name can appear, and a grep that misses it reports 12 false gaps — parity exits 0 (12 named divergences), 203 shared tests green. 🟡 **NOT ON A HANDSET.** Not one line of 7, 8, 8b or the OTP work. The last thing walked was 16 September, and §2's rule — do not ship a partial tour — now turns on a WALK, not on more building |
...
> 1. **Founder decisions:** ✅ ALL ANSWERED 2026-09-18 — Q8 (set-up model, app. 02), `?version=`, the trial loophole (`trial·ledger`), no archiving an attached lesson, no subject removal, DLT parked; ~~`?version=`~~ ✅ answered 2026-09-18 — the notice's own version line at its foot is enough; no "version you were shown" view is wanted, so the client prop stays unused on both surfaces (the server parameter stays, harmless); plus the three below (trial reset, archiving an attached
>    plan, DLT).
> 2. ~~**Needs the running web at 390px**~~ ✅ DONE 2026-09-18 (§0 `measure·390`).
...
✅ **DONE 2026-09-18 — see `map·01-02` in §0: neither family was hiding a feature.** ~~The largest UNKNOWN, and the best next piece of work after the walk: appendices 01 and 02 have never
...
**Small and specific, each already named in its own row** — ✅ **2026-09-18: all but two DONE** (the
prepare-picker dead end, the export's missing subject, the web's set-up-check reach, the entitlement
script's scope check and the dead `AccountPanel.jsx`); **still open: `?version=` (now a question — see
below) and the 390px re-measure of the `7·clusters` theme keys.** Original list:
...
`packages/shared` (209). ⚠️ (2026-09-18: `check-tdz.mjs` joined them — a const read before its declaration is now caught.) **None of them can see an UNDEFINED IMPORT** — `lib/tour.js` imported a
...
- ~~**Reports modal** (C30-C36)~~ — **DONE 2026-09-17**, `components/ReportSheet.jsx` over the app's one
  ...
  ★ **And a PDF is SHOWN before it is sent** (§0 `7·preview`) — `app/(app)/preview.jsx`. A phone-only
  addition, not a web divergence: the web's anchor download lands in a Downloads folder the browser
  already shows her, and there is nothing for the web to owe.
...
### Step 8 — LessonView residuals and the parity debt  🟡 **LARGELY DONE 2026-09-17 — the walk is what is left**
...
### Step 8b — the GUIDED TOUR  🟡 **BUILT 2026-09-17 — the machinery and all 20 steps; the DEMO ORCHESTRATION and the walk are what remain**
...
5. **`finishTour`** — the single exit for Done AND Skip — closes Ask, clears the tour, session-dismisses
   and goes to My Classes. ★ **IT DOES NOT RAISE THE CHECK WINDOW (founder, 2026-09-17).** The web raises
   it here and the phone will not, and this is a **named divergence with a reason**: since 2026-09-16 the
   phone already raises that window from a one-shot first run leaves (`lib/firstRun.js` →
   `(app)/index.jsx`, §0 row 6a), so keeping the web's trigger too would ask her twice. Asked which to
   retire, the founder kept the phone's. ★ **The reasoning is worth carrying to the WEB:** the tour's
   ending can only fire for a teacher who RAN the tour, yet Meyy assumed exactly as much for the one who
   skipped it — a section, a periods a week, a year's total — so on the web that disclosure is still
   missed by everyone who declines the tour. **Web owed** (recorded, not done): move the web's trigger to
   the same first-run one-shot. `shared/setupCheck.js` (F3, `65c1bf1d`) already holds the queue.
...
| ~~Q18~~ | 8 | ✅ **ANSWERED 2026-09-17 — KEEP THE BEHAVIOUR, RENAME THE BUTTON.** It is now **Dictate** on both surfaces, with an accessibility label that says what actually happens ("Start writing — then use your keyboard's mic to dictate"). ★ The founder took neither the status quo nor a real recogniser: a mic labelled "Speak" promises that Meyy is listening, and a teacher who speaks at it and finds nothing written learns that this app's buttons cannot be taken at their word. A real recogniser was declined for now — it needs mic permission and a DEV BUILD, and would have forced that milestone early. ⚠️ **On the WEB the control is close to a no-op** (focusing a textarea offers a desktop user nothing, and there is no keyboard mic to point at) — whether it should exist there at all is recorded as a separate call, not taken. [06·1] |
| ~~Q19~~ | 8 | ✅ **ANSWERED 2026-09-17 — DROP IT; PINE EVERYWHERE, AND THE WEB MOVED.** ★ This is the rare one where the PHONE's simplification won and the web changed to match: 24 uses of `#0f6e56` in globals.css became `var(--pine)`. The distinct shade was a holdover from when assessment was a full-screen sub-view with its own green box and card chrome — both retired 2026-07-10 — so by September it marked a distinction the UI no longer draws. ⚠️ The paired tint `#eef6f1` is deliberately untouched: it is a background, never the accent in question, and it holds MORE contrast against pine than against the old green. If assessment ever needs to read as its own place again, do it with structure, not a near-identical hue. [06·5] |
| ~~Q20~~ | 8 | ✅ **ANSWERED 2026-09-17 — NEITHER. BOTH ARE NAMED DIVERGENCES.** The founder took the third option on both halves. ⓵ **Notes placement**: the phone opens the panel below its top bar (a panel covering the app's own nav leaves one way out of itself — Ask Meyy's scrim rule), the web keeps it centred (no bar worth preserving, and a centred card reads as the modal it is). Named in `ChapterOrg.jsx` AND `web/app/components/LessonView.jsx`, each pointing at the other. ⓶ **The error-screen copy** is a phone ADDITION under CLAUDE.md §0's second allowance, not a gap: the web fails silently, the phone says so with a way back. ⚠️ Do not "fix" either surface to match the other without asking again — and **the web still owes its teacher an answer on the failure case**. [06·2, 06·3] |
