...
| 5 | `writePointer(i)`: clamp, set, store, **`setDone(false)` if moved off last unit**, push | W:2032-2039 | PARTIAL | M:250 via SS:187 `setUnitPointer` | shared | Mobile does NOT clear `lu_done_` when the pointer moves back below the last unit. Reachable only via cross-device drift / `Reopen` paths; add `if (n < total-1) setDone(false)` to match. |
| 6 | Pointer stored as string; `0` stored as "0" | W:2035 | PARTIAL | SS:190 | — | Shared `setUnitPointer` REMOVES the key at 0 ("mirror pullSectionState"). Reader-equivalent, but the web's own `writePointer` still writes "0" — the two apps disagree on disk shape for the same state; harmless, worth unifying by moving the web onto the shared helpers (plan says "web keeps its inline copy for now"). |
...
| 14 | Loading copy while opening a plan | MyPlans.jsx:595 "Opening plan…" | PARTIAL | R:39 "Opening the lesson…" | | String differs. Belongs to the My Classes family's row list too; flagging here since it is in lesson.jsx. |
| 15 | Open-failure copy | MyPlans.jsx:294 `.catch(() => {})` (silent) | PARTIAL (mobile-only addition) | R:29-30 "This lesson could not be found." / "Couldn't load this lesson right now." / "Nothing to show." / "‹ Back" | | Web fails silently (stays on the list); mobile shows an error screen with strings the web never says. Better behaviour, but it is copy the web does not have — founder call whether the web adopts it. |

### B. Chapter Organization (front door)

| # | Feature / UI element | Web ref | Mobile status | Mobile ref | Depends on | Notes |
|---|---|---|---|---|---|---|
| 16 | Wrapper `.lessonview.co-view[data-subject]` (drives science/SS-specific CSS) | W:1821 | PARTIAL | CO:403; `tight` prop CO:383 | | `data-subject` CSS does three things at ≤600px: `.co-utitle` 10.5px for science/SS (ported as `tight`), `.co-acc-name` 13px for SS (NOT ported — mobile 14.5 for all), `.co-count` hidden for science/SS (NOT ported — mobile always shows the count). |
...
| 21 | Tick rail `.co-rail` (tracking, not ssFlow) w/ `aria-label "{taught} of {total} units taught"`; tick done=pine, cur=OCHRE, else `--line` | W:1841-1847; css 3735-3737 | PARTIAL | CO:412-416; WJ:266-269 | WJ `co_rail/co_tick*` | Colour drift: web `.co-tick.cur` is `--ochre`; mobile `co_tick_cur` is `t.clay`. Base tick web `--line`, mobile `t.card_tick`; height 5 vs 4, gap 2 vs 3. Checker does not compare colour. |
...
| 26 | Accordion head: label (science section number stripped; SS competency truncated to 12 words while closed), `.co-count`, SVG chevron rotating 180° | W:1910-1929 | PARTIAL | CO:435-443 | WJ `co_acchead/co_acc_name/co_count` | Chevron is a "⌄" text glyph rotated, not the web's 12×12 SVG path; count not hidden for science/SS on phone (row 16). |
...
| 30 | Maths-prep flat list, >4 units → capped scroll window `.co-flatwrap/.co-flatscroll(max-h 348 phone)/.co-flatfade` | W:1696-1704, 1866-1901 | PARTIAL | CO:425-429 | | Mobile renders the flat cards in the page ScrollView with NO 4-unit window and NO bottom fade. Not named as a divergence in the header. Native answer: a fixed-height inner ScrollView (`maxHeight: 348`) + a LinearGradient/absolute View fade. |
| 31 | scrollIntoView of the "now" card on open (tracking; accordion or SS map) | W:1679-1687 | MISSING | — | ScrollView ref + onLayout y | Native: capture the `cur` card's `onLayout` y and `scrollRef.scrollTo({y, animated:false})` after mount. Matters for long SS/science chapters. |
...
| 46 | `cnSubjectGrade`: `CN_ROMAN` array I…XII | W:1257-1265 | PARTIAL | CO:47-53 | | Mobile map covers only 3–10; classes I, II, XI, XII would print the digit. Aruvi serves III–X today, so latent. Better: import a shared Roman helper. |
...
| 75 | Rest position = centre of phase i's `.uv-ph-time` (measured, re-measured on layout/resize) | W:184-202 | DONE (translation) | M:128-129, 152-155; PB:26-32 | onLayout | Mobile centre = `rows[i].y + 13 + timeH/2` — the `13` is `uv_phase.paddingVertical` hard-coded in the panel; if WJ:182 changes, the arrow drifts. Derive from `ws.uv_phase.paddingVertical`. |
...
| 79 | `.dragging` lift (scale 1.14 + shadow), hover shadow, `focus-visible` ring, `touch-action:none` | css 4586-4607 | PARTIAL | PB:58 `opacity .85` | | Drag affordance differs (opacity vs scale+shadow). Skin. |
...
| 81 | On genuine unit change: restore saved phase for that unit, else reset to 0 AND `writeLocalBookmark(sk, cur, 0)` so the server row agrees | W:2071-2087 | PARTIAL | M:248 | shared | Mobile effect runs on `[cur]` (incl. mount — harmless, read-only) but never WRITES the reset. After "Mark unit complete" the server bookmark still names the previous unit until she drags. Add the `writeLocalBookmark(sectionKey, cur, 0)` branch. |
| 82 | `moveBookmark(phase)` → state + `writeLocalBookmark` (pushes) | W:2088-2091 | DONE | M:257 | | |
| 83 | Cross-device resync: re-read bookmark on `window focus` / `pageshow` / `storage` | W:2092-2113 | MISSING | — | `AppState` (react-native) | Native answer: `AppState.addEventListener("change", s => s === "active" && resync())`, optionally preceded by `pullSectionState([sectionKey])` so the phone that was backgrounded adopts the laptop's move. Also a good place to re-pull the pointer. |
...
| 86 | Just-marked: `.lv-donecard` "✓" (26px filled green circle, white) · "Unit complete" (pine-d 600) · "↺ Undo" (white pill w/ edge-green border) | W:2280-2289; css 1553-1559 | PARTIAL | M:313, `DoneCard` 342-350 | WJ `lv_donecard/lv_donemark/lv_donetitle/lv_undo` | Skin: `lv_donemark` is plain 18px pine text (web: 14px white on a 26px #2f7d54 disc); `lv_donetitle` ink 500 (web pine-d 600); `lv_undo` bare text (web a bordered white pill, 44px min-height on phone). Checker caught `lv_donemark` 14 vs 18 and `lv_undo` tracking; the disc/pill boxes are outside its scope. |
| 87 | Chapter done: `.lv-chapterdone` (tint-clay bg, edge-clay border, mark clay, title clay) "Chapter complete" · "↺ Reopen" (`setDone(false)`) | W:2290-2301; css 1562-1564 | PARTIAL | M:315; WJ:196 | | WJ `lv_chapterdone` = ochre border + `tint_cream`; web = `--tint-clay` + `--edge-clay`, and the mark/title recolour to clay is not applied. |
...
| 93 | `AssessBody key={idx}`; `qn` only when >1 item; `Q{n}.` FLOATED mark sharing the panel's first line (`.assess-qmark` pine italic 13) | W:415-416, 1185-1189; css 1499 | PARTIAL | AP:440, 381, 396; WJ:228 | | Mobile renders the mark as its own block line, and in CLAY (web: `var(--pine)`) at 15px (web 13). Colour miss is outside the checker's scope; size is in its list. |
| 94 | Legacy item (no `normalized.template`): white `.assess-card` with LEARNING OUTCOME strip + `ALegacyCard` (type name, prompt, `<ol>` options, "Answer: …", LOOK FOR) | W:1190-1203, 732-749 | PARTIAL | AP:382-388, 361-371 | WJ `assess_qtype/assess_lo_k`; no `assess_card` | Card chrome (#fff, edge-green border, radius 10) not ported — items sit flat. Options numbered "1." (web `<ol>` decimal) ✓. |
| 95 | Forward nav "Question →" / "Answer →" (only when target tab exists) and "Next question →" (Answer + Inclusivity, not last item): inline `<span role=button tabIndex=0>` riding the last row, Enter/Space keys | W:1179-1184, 1212-1217; css 1485, 1518 | PARTIAL (keys WEB-ONLY-BY-NATURE) | AP:373-375, 380, 393 | WJ `assess_tabnav` | Pressable ✓; but always on its OWN right-aligned line (`alignSelf:flex-end`), never sharing the last row as the web's flex-wrap does. Founder 2026-07-16 asked for the ride-the-row behaviour. |
...
| 101 | `audio_ref` cue: 5-bar equaliser SVG icon + "*Listening passage* — {ref}, read aloud" in a pine-left-ruled row | W:899-908; css 1719-1723 | PARTIAL | AP:212-214 | | Text verbatim; the SVG icon and the 3px pine left rule are NOT ported; `audio_ref` mono 11 not applied. No web.js `assess_audio*` keys — PARITY-CHECK OWED. |
...
| 105 | prose → `.assess-vs.assess-vs-prose` (green tint box, pre-line) | W:547 | PARTIAL | AP:146 | | Mobile plain `P` — no box. |
...
| 124 | `requestAnimationFrame` + `document.querySelector(".co-view .co-card.cur, .co-view .cof-u.cur").scrollIntoView` | W:1681-1683 | MISSING | Row 31. |
| 125 | `window.addEventListener("focus"/"pageshow"/"storage")` bookmark resync | W:2104-2110 | MISSING | Row 83; `AppState`. |
...
| 135 | Competency dots coloured by identity colour | CO:180 | Row 37; web dots are `--ink`. |
